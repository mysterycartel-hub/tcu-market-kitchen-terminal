...
// Which roadmap step gates each overlay
...
export default function PracticeChart({ checkedSteps }: PracticeChartProps) {
  // No prop = show all overlays (standalone use)
  const show = (key: keyof typeof GATE) =>
    !checkedSteps || !!checkedSteps[GATE[key]];

  const noneChecked =
    !!checkedSteps && !Object.values(checkedSteps).some(Boolean);

  return (
    <div className="relative w-full h-full flex flex-col bg-[#050c1a] rounded-xl border border-white/10 overflow-hidden">
      {noneChecked && (
        <div className="pointer-events-none absolute top-3 left-1/2 -translate-x-1/2 z-10 rounded-full border border-violet-500/40 bg-violet-900/80 px-4 py-1.5 text-[11px] font-bold text-violet-200 tracking-wider whitespace-nowrap">
          Check recipe steps on the right to reveal overlays →
        </div>
      )}
      <svg
      ...
      >
        ...
        {/* Watermark */}
        <text x="530" y="230" textAnchor="middle" fill="rgba(148,163,184,0.04)" fontSize="130" fontWeight="900" letterSpacing="8">TCU</text>
        ...
        {TIME_AXIS.map(({ x, label }) => (
          <g key={label}>
            ...
            <text x={x + CW / 2} y="548" textAnchor="middle"
              fill={label === "8:00AM" ? "rgba(168,85,247,0.9)" : "rgba(148,163,184,0.5)"}
              fontSize="10" fontFamily="monospace" fontWeight={label === "8:00AM" ? "bold" : "normal"}
            >{label}</text>
          </g>
        ))}

        {/* ── OVERLAY: Bias — session band + bias label ── */}
        {show("bias") && (
          <g>
            ...
            <rect x="870" y="42" width="140" height="22" fill="rgba(16,185,129,0.2)" rx="3" />
            <text x="940" y="57" textAnchor="middle" fill="rgba(110,231,183,0.95)" fontSize="11" fontWeight="bold">Bias: Bullish ✓</text>
          </g>
        )}

        {/* ── OVERLAY: Liquidity — sweep marker + structure lines ── */}
        {show("liquidity") && (
          <g>
        ...
        {show("aoi") && (
          <g>
            ...
            <text x="622" y="244" textAnchor="middle" fill="rgba(216,180,254,0.9)" fontSize="11" fontWeight="bold">AOI ✓</text>
            <text x="622" y="260" textAnchor="middle" fill="rgba(192,132,252,0.7)" fontSize="9">Prep Zone</text>
          </g>
        )}

        {/* ── OVERLAY: Delivery — FVG / imbalance zone ── */}
        {show("delivery") && (
          <g>
            ...
            <text x="514" y="188" textAnchor="middle" fill="rgba(147,197,253,0.95)" fontSize="11" fontWeight="bold">FVG ✓</text>
            <text x="514" y="203" textAnchor="middle" fill="rgba(96,165,250,0.75)" fontSize="9">Delivery Vehicle</text>
          </g>
        )}

        {/* ── OVERLAY: Confirmation — highlight candle ── */}
        {show("confirmation") && (
          <g>
            <rect x="650" y="205" width="30" height="70" fill="rgba(168,85,247,0.15)" stroke="rgba(168,85,247,0.6)" strokeWidth="1.5" rx="2" />
            <text x="665" y="200" textAnchor="middle" fill="rgba(216,180,254,0.95)" fontSize="10" fontWeight="bold">Confirm ✓</text>
          </g>
        )}

        {/* ── OVERLAY: Entry — entry zone line ── */}
        {show("entry") && (
          <g>
            <line x1="658" y1="218" x2="1060" y2="218" stroke="rgba(250,204,21,0.5)" strokeWidth="1.5" strokeDasharray="8,5" />
            <circle cx="658" cy="218" r="6" fill="rgba(250,204,21,0.85)" />
            <rect x="670" y="210" width="95" height="16" fill="rgba(250,204,21,0.15)" rx="2" />
            <text x="717" y="222" textAnchor="middle" fill="rgba(253,224,71,0.95)" fontSize="10" fontWeight="bold">Entry Zone ✓</text>
          </g>
        )}

        {/* ── OVERLAY: Targets — Tables Served T1 / T2 ── */}
        {show("targets") && (
          <g>
            ...
            <rect x="840" y="100" width="130" height="16" fill="rgba(16,185,129,0.15)" rx="2" />
            <text x="905" y="112" textAnchor="middle" fill="rgba(110,231,183,0.9)" fontSize="10" fontWeight="bold">Tables Served ✓</text>
          </g>
        )}

        {/* ── OVERLAY: Management — Burn Point / invalidation ── */}
        {show("management") && (
          <g>
            ...
            <rect x="100" y="476" width="108" height="18" fill="rgba(239,68,68,0.15)" rx="2" />
            <text x="154" y="488" textAnchor="middle" fill="rgba(252,165,165,0.9)" fontSize="10" fontWeight="bold">Burn Point ✓</text>
          </g>
        )}
        ...
        {show("liquidity") && (
          <><circle cx="118" cy="566" r="5" fill="rgba(251,191,36,0.7)" /><text x="128" y="570" fill="rgba(253,224,71,0.8)" fontSize="9" fontWeight="bold">Gold=Liquidity</text></>
        )}
        {show("aoi") && (
          <><circle cx="238" cy="566" r="5" fill="rgba(168,85,247,0.7)" /><text x="248" y="570" fill="rgba(216,180,254,0.8)" fontSize="9" fontWeight="bold">Purple=AOI</text></>
        )}
        {show("delivery") && (
          <><circle cx="338" cy="566" r="5" fill="rgba(59,130,246,0.7)" /><text x="348" y="570" fill="rgba(147,197,253,0.8)" fontSize="9" fontWeight="bold">Blue=FVG</text></>
        )}
        {show("management") && (
          <><circle cx="418" cy="566" r="5" fill="rgba(239,68,68,0.7)" /><text x="428" y="570" fill="rgba(252,165,165,0.8)" fontSize="9" fontWeight="bold">Red=Burn Pt</text></>
        )}
        {show("targets") && (
          <><circle cx="528" cy="566" r="5" fill="rgba(34,197,94,0.7)" /><text x="538" y="570" fill="rgba(110,231,183,0.8)" fontSize="9" fontWeight="bold">Green=Tables Served</text></>
        )}
        <text x="900" y="570" fill="rgba(148,163,184,0.4)" fontSize="8">Practice chart · TCU educational model · Not live data</text>
      </svg>
    </div>
  );
}
